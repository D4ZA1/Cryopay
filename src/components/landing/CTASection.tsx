    ...
    <section className="cta-section" ref={ref}>
      ...
      <motion.div style={{ maxWidth: 800, margin: '0 auto', position: 'relative', zIndex: 10 }} initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          <motion.button className="btn-primary btn-lg" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => navigate('/connect-wallet')}>Create Your Wallet</motion.button>
          <motion.button className="btn-outline" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>Find Smart Bins</motion.button>
        </div>
