 ...
 */
export type TransactionType = 'Sent' | 'Received' | 'Buy' | 'Sell';

/**
 * Parsed transaction for frontend display
 * @description Combines raw block data with computed display properties
 */
export interface Transaction {
  ...
  to: string;
  /** Recipient display name (resolved from contacts or profile) */
  toDisplayName: string;
  /** Sender address/identifier */
  from: string;
  /** Sender display name (resolved from contacts or profile) */
  fromDisplayName: string;
  /** Formatted date string */
  date: string;
  /** Amount in USD (or primary fiat currency) */
  amountUSD: number;
  /** Amount in cryptocurrency */
  amountCrypto: number;
  ...
  raw: BlockOutput;
  /** Whether this transaction is relevant to the current user */
  relevant: boolean;
}

/**
 * Block type alias for frontend use
 */
export type Block = BlockOutput;

/**
 * JWK (JSON Web Key) structure for cryptographic keys
 */
...
/**
 * Encrypted private key blob structure
 */
...
/**
 * Public key with thumbprint for identification
 */
export interface PublicKeyWithThumbprint extends JWK {
  thumbprint: string;
}

/**
 * Wallet state for frontend context
 */
export interface WalletState {
  ...
  /** Whether the wallet is verified */
  isVerified: boolean;
  /** Public key JWK (parsed) */
  publicKey: JWK | null;
  /** Public key thumbprint for identification */
  thumbprint: string | null;
  /** Whether the private key is available (decrypted) */
  hasPrivateKey: boolean;
}

/**
 * User profile state for frontend context
 */
